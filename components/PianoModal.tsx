...
export const PianoModal: React.FC<PianoModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div
      className={`fixed inset-0 bg-black bg-opacity-60 z-50 flex justify-center items-center p-2 sm:p-4 transition-opacity duration-300 ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
      onClick={onClose}
      role="dialog"
      aria-modal="true"
    ...
    >
      <div
        className={`bg-white dark:bg-[#171725] rounded-xl shadow-2xl w-full max-w-screen-xl max-h-[95vh] flex flex-col text-slate-800 dark:text-slate-200 transform transition-all duration-300 ${isOpen ? 'scale-100 opacity-100' : 'scale-95 opacity-0'}`}
      ...
      >
        ...
        <div className="p-2 sm:p-4 overflow-y-auto">
            <p className="text-center text-sm text-slate-500 dark:text-slate-400 mb-2">
                Click, drag, or use your keyboard to play.
            </p>
            <Piano />
        </div>
      </div>
