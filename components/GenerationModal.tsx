                ...
                <div className="w-full sm:w-[60%] md:w-2/3 sm:pl-6 flex flex-col flex-grow min-h-0 overflow-hidden">
                    {activeMode !== 'pixshop' ? (
                       ...
                       <div className="flex flex-col h-full overflow-hidden">
                           ...
                           <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-4 mt-4 border-t border-slate-200 dark:border-slate-700 flex-shrink-0">
                               <div className="space-y-2"><h4 className="font-semibold text-sm">Quick Edits</h4>
                                   <div className="grid grid-cols-2 gap-2 text-sm">
                                       <button onClick={() => { if(!isCropping) setIsCropping(true); }} disabled={!pixshopImage || isLoading} className={`tool-btn ${isCropping ? '!bg-indigo-500 text-white' : ''}`}><CropIcon className="w-4 h-4"/> Crop</button>
                                   ...
                                   {isCropping && <div className="grid grid-cols-2 gap-2 text-sm"><button onClick={handleApplyCrop} disabled={!cropRect} className="tool-btn bg-green-500 text-white hover:bg-green-600">Apply Crop</button><button onClick={() => { setIsCropping(false); setCropRect(null); }} className="tool-btn bg-red-500 text-white hover:bg-red-600">Cancel</button></div>}
                               ...
                               <div className="space-y-2"><h4 className="font-semibold text-sm">Advanced Tools</h4>
                                   <div className="grid grid-cols-2 gap-2 text-sm">
                                   ...
                                   <select onChange={(e) => handlePixshopEdit(e.target.value)} disabled={!pixshopImage || isLoading} className="tool-btn w-full">
                                       <option>Apply Artistic Style...</option>
                                       {pixshopArtStyles.map(s => <option key={s.name} value={s.prompt}>{s.name}</option>)}
                                   </select>
                               </div>
                               <div className="space-y-2"><h4 className="font-semibold text-sm">Adjustments</h4>
