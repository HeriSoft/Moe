...
export const GenerationModal: React.FC<GenerationModalProps> = ({ isOpen, onClose, userProfile, setNotifications, onProFeatureBlock, handleExpGain, setUserProfile }) => {
    ...
    const getOriginalOutputSettings = useCallback((): ImageEditingSettings => {
        // FIX: If using Gemini 3 Pro, DO NOT constrain output dimensions based on input image.
        // Let the model and the aspectRatio setting handle the dimensions.
    ...
    const handleApplyAdvancedStyles = () => {
        ...
        // --- FIX START: Logic tạo prompt thông minh hơn ---
        ...
        if (outfitsToApply.length > 0) {
            ...
            if (isMixOutfit && outfitsToApply.length > 1) {
                 basePrompt += " The SUBSEQUENT images are OUTFIT REFERENCES. Create a new outfit for the subject that is a CREATIVE MIX of the styles shown in the reference images.";
            } else {
                 basePrompt += " The SECOND image is the OUTFIT REFERENCE. Change the subject's outfit to match the design, style, and texture of this reference outfit exactly.";
            }
        }
        ...
        if (isCustomPose) {
            if (usePoseRefImage && poseRefImage) {
                promptParts.push("Recreate the pose of the subject to match the pose in the provided reference image.");
                additionalImages.push(poseRefImage); // Add pose ref image
            } else if (customPosePrompt) {
                promptParts.push(`Change the subject's pose to: ${customPosePrompt}.`);
            }
        } else if (selectedPose) {
            promptParts.push(`Change the subject's pose to: ${selectedPose}.`);
        }

        // 4. Xử lý Expression & Background
        if (selectedExpression) promptParts.push(`Change the facial expression to: ${selectedExpression}.`);
        if (backgroundPrompt) promptParts.push(`Change the background to: ${backgroundPrompt}.`);

        // 5. Xử lý Aspect Ratio (Fix lỗi hình vuông cho Gemini 3 Pro)
        // Nếu người dùng chọn ratio khác Auto và khác 1:1 (mặc định), hãy nhắc model fill vào khoảng trống
        if (editSettings.aspectRatio && editSettings.aspectRatio !== 'auto' && editSettings.aspectRatio !== '1:1') {
             promptParts.push(`Generate the output in ${editSettings.aspectRatio} aspect ratio. Extend the background naturally to fill the new frame (Outpainting).`);
        }
        ...
        // Prompt phụ trợ để giữ chất lượng
        constructedPrompt += " Ensure high quality, photorealistic lighting, and seamless blending of the new outfit.";
        // --- FIX END ---
        
        handleGenericApiCall(async () => {
             const finalEditSettings = getOriginalOutputSettings();
             // Important: The inputImage1 is always the first image in the array
             const { attachments } = await editImage(constructedPrompt, [inputImage1, ...additionalImages], finalEditSettings, userProfile);
             
             // Reset states after generation
    ...
    const toggleOutfitSelection = (fileName: string) => {
        if (isMixOutfit) {
            setSelectedOutfits(prev => prev.includes(fileName) ? prev.filter(name => name !== fileName) : (prev.length < 3 ? [...prev, fileName] : prev));
        } else {
            setSelectedOutfits(prev => (prev.includes(fileName) ? [] : [fileName]));
        }
    };
                                                    ...
                                                    <div className="grid grid-cols-2 gap-4">
                                                        <div>
                                                            ...
                                                            {isCustomPose ? (
                                                                ...
                                                                <div className="grid grid-cols-2 xl:grid-cols-4 gap-2 text-xs">{POSES.map(p => <button key={p.label} onClick={() => setSelectedPose(prev => prev === p.prompt ? null : p.prompt)} className={`p-2 bg-slate-200 dark:bg-slate-700 rounded-md hover:bg-indigo-100 dark:hover:bg-indigo-900/50 ${selectedPose === p.prompt ? 'ring-2 ring-indigo-500' : ''}`}>{p.label}</button>)}</div>
                                                            )}
                                                        ...
                                                        <div>
                                                            ...
                                                            <div className="grid grid-cols-3 gap-2">{[...Array(6)].map((_, i) => userOutfits[i] ? <button key={userOutfits[i].fileName} onClick={() => toggleOutfitSelection(userOutfits[i].fileName)} className={`relative rounded-md overflow-hidden aspect-square ${selectedOutfits.includes(userOutfits[i].fileName) ? 'ring-2 ring-indigo-500' : ''}`}><img src={`data:${userOutfits[i].mimeType};base64,${userOutfits[i].data}`} className="w-full h-full object-cover"/><button onClick={(e) => { e.stopPropagation(); setUserOutfits(prev => prev.filter((_, idx) => idx !== i)); }} className="absolute top-1 right-1 p-0.5 bg-black/50 text-white rounded-full"><TrashIcon className="w-3 h-3"/></button></button> : <div key={i} className="flex flex-col gap-1 items-center justify-center p-1 rounded-md border-2 border-dashed border-slate-300 dark:border-slate-600 aspect-square"><button onClick={handleAddOutfitFromDrive} className="p-1 rounded-full bg-slate-200 dark:bg-slate-700"><GoogleDriveIcon className="w-3 h-3"/></button><input type="file" id={`outfit-upload-${i}`} onChange={handleAddOutfit} className="hidden"/><label htmlFor={`outfit-upload-${i}`} className="p-1 rounded-full bg-slate-200 dark:bg-slate-700 cursor-pointer"><ArrowUpTrayIcon className="w-3 h-3"/></label></div>)}</div>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div className="space-y-4">
                                                    <div><h4 className="text-sm font-semibold mb-2">Expression</h4><div className="grid grid-cols-4 xl:grid-cols-7 gap-2">{EXPRESSIONS.map(e => <button key={e.label} onClick={() => setSelectedExpression(prev => prev === e.prompt ? null : e.prompt)} title={e.label} className={`p-2 bg-slate-200 dark:bg-slate-700 rounded-md hover:bg-indigo-100 dark:hover:bg-indigo-900/50 flex justify-center items-center ${selectedExpression === e.prompt ? 'ring-2 ring-indigo-500' : ''}`}><e.Icon className="w-5 h-5"/></button>)}</div></div>
