...
export const GenerationModal: React.FC<GenerationModalProps> = ({ isOpen, onClose, userProfile, setNotifications, onProFeatureBlock, handleExpGain }) => {
    ...
    const handleAddOutfit = (file: File) => {
        if (userOutfits.length >= 5) { setNotifications(p => ["Maximum of 5 outfits saved.", ...p]); return; }
        const reader = new FileReader();
        reader.onloadend = () => {
            const base64String = (reader.result as string).split(',')[1];
                    ...
                    <div className="flex-grow flex flex-col lg:flex-row gap-6 py-4 min-h-0 overflow-y-auto">
                        ...
                        <div className="w-full lg:w-1/2 flex flex-col gap-4">
                            ...
                            <div className="bg-slate-100 dark:bg-[#2d2d40] rounded-lg flex items-center justify-center p-2 min-h-[250px] lg:min-h-0 lg:flex-grow">
                                ...
                                {!isLoading && !error && output.length > 0 && <div className={`grid gap-2 w-full ${output.length > 1 ? 'grid-cols-2' : 'grid-cols-1'}`}>{output.map((item, index) => <div key={index} className={`relative group w-full ${getAspectRatioClass()}`}><img src={`data:${item.mimeType};base64,${item.data}`} alt="Generated media" className={`rounded-lg object-contain w-full ${getAspectRatioClass() ? 'h-full' : 'h-auto'}`}/><button onClick={() => handleDownload(item)} className="absolute top-2 right-2 p-2 rounded-full bg-black/50 text-white opacity-0 group-hover:opacity-100 focus:opacity-100"><DownloadIcon className="w-5 h-5"/></button></div>)}</div>}
