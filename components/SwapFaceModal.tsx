  ...
  return (
    <div className="flex flex-col">
      ...
      <button
        ...
        className="w-full aspect-square bg-slate-100 dark:bg-[#2d2d40] rounded-lg border-2 border-dashed border-slate-300 dark:border-slate-600 flex flex-col items-center justify-center text-slate-500 dark:text-slate-400 hover:border-indigo-500 hover:text-indigo-500 transition-colors p-2 text-center"
      >
        {image ? (
...
export const SwapFaceModal: React.FC<SwapFaceModalProps> = ({ isOpen, onClose, setNotifications, userProfile, onProFeatureBlock }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex justify-center items-center" onClick={onClose} role="dialog" aria-modal="true" aria-labelledby="swapface-title">
      <div className="bg-white dark:bg-[#171725] rounded-xl shadow-2xl w-full max-w-4xl p-4 sm:p-6 m-4 text-slate-800 dark:text-slate-200 flex flex-col max-h-[90vh]" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex flex-col md:flex-row gap-8 overflow-y-auto pr-2 -mr-2">
            ...
            <div className="w-full md:w-1/2 flex flex-col">
                <h3 className="text-lg font-semibold text-center mb-2 text-slate-800 dark:text-slate-200">Result</h3>
                <div className="w-full aspect-square bg-slate-100 dark:bg-[#2d2d40] rounded-lg flex items-center justify-center">
                    ...
                    {!isLoading && !resultImage && (
                    <div className="flex flex-col items-center text-center text-slate-500 dark:text-slate-400 p-4">
                        <PhotoIcon className="w-16 h-16" />
                        <span className="mt-2 text-sm font-semibold">Result will appear here</span>
                    </div>
                    )}
                </div>
            </div>
        ...
        {error && <p className="flex-shrink-0 text-center text-red-500 bg-red-500/10 p-3 rounded-lg mt-6">{error}</p>}
